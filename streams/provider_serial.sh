...
for r in $(seq $first_router $last_router); do
	...
	#sudo qm stop ${provider}0${provider}00${r}
...
for r in $(seq $first_router $last_router); do 
    ansible-playbook -i inventories/inventory${provider}.yaml remove_images.yml "-l p${provider}r${r}v"
done

# Show remaing image
echo -e "${C}Remaining image on router$([[ $first_router != $last_router ]] && echo s):${NC}"
for r in $(seq $first_router $last_router); do 
    ansible-playbook -i inventories/inventory${provider}.yaml show_image.yml "-l p${provider}r${r}v"
done
...
do 
    ansible-playbook -i inventories/inventory${provider}.yaml setup.yml "-l p${provider}r${r}v"
done
